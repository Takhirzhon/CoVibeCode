...
impl SessionActor {
    ...
    async fn handle_respond_permission(
        ...
        // If asked to remember, and this is an allow, record the request's tool so
        // future prompts for it auto-allow. Capture the tool name before clearing the
        // pending entry (clear removes it from the map).
        if remember_tool && response.get("behavior").and_then(|v| v.as_str()) == Some("allow") {
            if let Some(tool) = self
                .pending_interactive_requests
                .get(request_id)
                .filter(|req| req.subtype == "can_use_tool")
