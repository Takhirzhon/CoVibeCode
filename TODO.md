...
- **Confirmed root cause (2026-07-08):** inspected the actual IDB snapshot blob for run `7f1ff9c5…` — its serialized `timeline` had **2 `kind:"user"` entries and 0 `kind:"assistant"`**, while the event log held all 212 assistant `message_complete` events. So the cached snapshot itself dropped the assistant turns; the snapshot fast-path (`loadRun` → `_tryApplySnapshot`) then rendered a user-only timeline. First load worked because it replays the (complete) event log.
- **Fix:** `_restoredSnapshotIsUsable()` sanity-checks the restored timeline; `loadRun` rejects a corrupt snapshot (user turns but no assistant/tool output, or a `_seenMessageIds` id with no surviving entry), deletes it, and falls back to a full event replay — which also rewrites a correct snapshot, so it self-heals. Regression test added (`falls back to replay when a valid snapshot dropped its assistant turns`).
- **Follow-up (optional):** find the *write*-side path that serialized an assistant-less timeline in the first place (self-healing fix covers the read side regardless).
- **Owner:** _unassigned_  ·  **Status:** `[x]` (fixed + tested; write-side root cause is an optional follow-up)
